  ...
  return (
    <>
      ...
      <div className="tbl-wrap">
        <table className="tbl">
          <thead><tr>
            <th>Emitido</th><th>Cliente</th><th>Projeto</th><th>Método</th><th>Vencimento</th><th>Estado</th><th className="num">Valor</th>
          </tr></thead>
          <tbody>
            {list.map(p => {
              ...
              return (
                <tr key={p.id} onClick={()=> t ? go({page:'tarefas', id:t.id}) : go({page:'clientes', id:c.id})}>
                  ...
                  <td data-l="Projeto">{t ? <a style={{color:'var(--ember)', fontWeight:600}}>{t.title}</a> : <span className="text-mute">{p.note || '—'}</span>}</td>
                  <td data-l="Método" style={{textTransform:'capitalize'}}>{p.method}</td>
